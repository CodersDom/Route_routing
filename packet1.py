...
class packet(object):
    def send(
        sequence_number,
    ):  # send function that runs in a separate thread, receives a global sequence number, the first time a router runs
        while True:
            sequence_number = (
                sequence_number.split(" ")[0]
                + " "
                + str((int(sequence_number.split(" ")[1]) + 1))
            )  # incrementing sequence number
            packet_contents = []  # packet to be sent

            alive_routers = []  # a list to check for alive routers
            while (
                routers_queue.empty() != True
            ):  # while the queue that contains alive routers is not empty
                alive_routers.append(
                    routers_queue.get()
                )  # get router names and append it to alive routers

            for (
                router,
                state,
            ) in routers_state.items():  # run a for loop on router_state list
                if (
                    router in alive_routers
                ):  # if a router in the list is also in the alive router list
                    lock.acquire()
                    routers_state[
                        router
                    ] = 1  # initialize that router as 1 (it is alive)
                    lock.release()
                    lock.acquire()
                    router_packets[
                        router
                    ] = 3  # and initialize router_packets to 3 as it is not dead
                    lock.release()
                else:  # if a router is not alive router list
                    lock.acquire()
                    router_packets[router] = (
                        router_packets[router] - 1
                    )  # decrement its counter making it closer to be delared as dead
                    lock.release()
            ...
            router_packets[
                router_id
            ] = 3  # and initializing its own router_packets id to 3
            lock.release()
            ...
            for (
                ...
                router_packets.items()
            ):  # run a loop on router_packets list to check which has the value less than or equal to 0
                if (
                    packets <= 0
                ):  # if a router's value is less than or equal to 0, it means that it is dead
                    lock.acquire()
                    routers_state[router] = 0  # initialize the router_state to 0 (dead)
                    lock.release()
                    received_packets.pop(
                        router, None
                    )  # and pop that router from the received packets list so we reset the sequence numbers of that router
            ...
            for (
                ...
                port_numbers
            ):  # iterating over all the neighbors and sending the packet to them
